...
function getAllGasPrice(geo) {
    return knex('gas_price_user').where("geo", geo).andWhere('indic_en', 'MSHH').select('year', 'value');
}
...
function getMaxElPrice(household, geo) {
    return knex('electricity_price_user').where("geo", geo).andWhere('indic_en', household).max('value').select('geo', 'year', 'value');
}
function getMinElPrice(household, geo) {
    return knex('electricity_price_user').where("geo", geo).andWhere('indic_en', household).min('value').select('geo', 'year', 'value');
}

function getMaxGasPrice(household, geo) {
    return knex('gas_price_user').where("geo", geo).andWhere('indic_en', household).max('value').select('geo', 'year', 'value');
}
function getMinGasPrice(household, geo) {
    return knex('gas_price_user').where("geo", geo).andWhere('indic_en', household).min('value').select('geo', 'year', 'value');
}
...
function getHouseholdUsageCountryFuel(geo, fuel) {
    return knex('household_by_fuel').where("geo", geo).andWhere('siec', fuel).select('geo', 'year', 'value')
}

function getHouseholdUsageYearFuel(year, fuel) {
    return knex('household_by_fuel').where("year", year).andWhere('siec', fuel).select('geo', 'year', 'value')
}

function getHouseholdUsageCountryYear(geo, year) {
    return knex('household_by_fuel').join('fuel_types','fuel_types.siec','household_by_fuel.siec').where("geo", geo).andWhere('year', year).select('fuel_types.fuel','geo', 'value')
}
